...
class Train:
    ...
    ## evaluation 
    def model_eval(self,eval_dataloader,evaldf):
        self.model.eval()
        loss = 0
...
## evaluation 
def model_eval(model,dataloader,df,idx2eid,device,cls_token_id):
    exclude = ['example_id','offset_mapping']
    model.eval()
    n = len(dataloader)
    ...
    for batch in dataloader:
        batch_ = {k: v.to(device) for k, v in batch.items() if k not in exclude}
        with torch.no_grad():
            outputs = model(**batch_)
        flat_batch(all_batches,batch)
        flat_outputs(all_outputs,outputs)

    df = data_postproc(evaldf,all_batches,all_outputs,idx2eid,cls_token_id)